...
export const fetchAsyncCreateCategory = createAsyncThunk(
    ...
    async (item: string) => {
        const res = await axios.post<CATEGORY>(
                `${process.env.REACT_APP_API_URL}/api/category/`, { item: item }, {
                headers: {
                    Authorization: `JWT ${localStorage.localJWT}`,
                }
            }
...
export const taskSlice = createSlice({
    ...
    extraReducers: builder => {
        ...
        builder.addCase(fetchAsyncGetCategory.fulfilled,
...
export const selectEditTask = (state: RootState) => state.task.editedTask
